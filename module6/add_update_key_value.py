...
class HashTable():
    ...
    def put(self, key, value):
        ...
        existing = self._find_key(index, key)

        if existing is not None:
            self.slots[existing].value = value
        else:
            free = self._find_free_slot(index)
            self.slots[free] = HashItem(key, value)
            self.used_slots += 1
